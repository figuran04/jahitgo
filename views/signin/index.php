...
<div class="flex flex-col md:flex-row w-full justify-center md:min-h-screen bg-[var(--primary-color)]">
    ...
    <div class="w-full md:w-2/3 md:mx-0 px-6 md:px-8 py-3 rounded-t-2xl md:rounded-none flex flex-col gap-3 md:justify-center bg-gray-50">
        <div class="flex flex-col md:flex-col xl:flex-row justify-between xl:items-center gap-3 xl:gap-0">
            <h2 class="text-[var(--primary-color)] text-2xl font-bold text-center md:text-left mt-2">Masuk ke JahitGo</h2>
            <div class="text-center md:text-left">
                <span>Belum punya akun? </span><a href="../signup<?= $url ? '?url=' . urlencode($url) : '' ?>" class="hover:text-[var(--primary-hover)]">Daftar</a>
            </div>
        </div>
        ...
        <form action="../../controllers/auth/signin_handler.php" method="POST" class="flex flex-col w-full gap-1">
            ...
            <input class="border rounded border-gray-300 p-2 outline-[var(--primary-color)]" type="email" id="email" name="email" placeholder="example@gmail.com" required>

            <label for="password">Password:</label>
            <input class="border rounded border-gray-300 p-2 outline-[var(--primary-color)]" type="password" id="password" name="password" placeholder="********" required>

            <a href="#" class="text-right text-sm my-2 hover:text-[var(--primary-hover)]">Butuh bantuan?</a>
